...
## ۱۸. راه‌اندازی ایمیل خروجی (اختیاری)

بخش EMAIL در `.env` **اختیاری** است: اگر خالی بماند سایت کامل کار می‌کند و
فقط «ارسال خبرنامه» و «ایمیل تأیید» غیرفعال می‌شوند. ورود کاربران با پیامک
است، نه ایمیل — پس چیز حیاتی‌ای از دست نمی‌رود و می‌توانید این مرحله را به
بعد موکول کنید.

هر وقت خواستید سایت ایمیل بفرستد، سه گزینه دارید:

### گزینه ۱ — جیمیل مخصوص کسب‌وکار (رایگان، ساده، پیشنهادی برای شروع)

یک حساب گوگل جدید مثل `oramshop.business@gmail.com` بسازید و از «رمز عبور
اپلیکیشن» (App Password) استفاده کنید — نه رمز اصلی حساب.

1. وارد حساب گوگل شوید → بخش Security → **تأیید دو‌مرحله‌ای (2-Step Verification)** را روشن کنید (بدون این، App Password ساخته نمی‌شود).
2. به [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords) بروید.
3. یک نام مثل `oramshop` بدهید و **Create** بزنید — یک رمز ۱۶ حرفی می‌دهد (مثل `abcd efgh ijkl mnop`). فاصله‌ها را حذف کنید.
4. در `.env` سرور:
...
EMAIL_PORT=587
EMAIL_USE_TLS=True
EMAIL_HOST_USER=oramshop.business@gmail.com
EMAIL_HOST_PASSWORD=abcdefghijklmnop   # همان رمز ۱۶ حرفی، بدون فاصله
```

> نکته: گیرنده‌ها فرستنده را همان آدرس جیمیل می‌بینند، نه info@oramshop.ir.
> برای شروع کاملاً قابل قبول است؛ محدودیت ارسال گوگل حدود ۵۰۰ ایمیل در روز
> است که برای یک فروشگاه نوپا بیش از کافی است.

### گزینه ۲ — ایمیل شخصی

دقیقاً مثل گزینه ۱، فقط با جیمیل شخصی خودتان (باز هم با App Password، نه رمز اصلی).

### گزینه ۳ — هاست ایمیل روی دامنه (وقتی جدی‌تر شدید)

اگر خواستید فرستنده `info@oramshop.ir` باشد، یک هاست ایمیل ایرانی بگیرید
(میل‌فا، ایران‌هاست، برتینا). آن‌ها SMTP و رکوردهای MX/SPF/DKIM را می‌دهند؛
مقادیر SMTP همان قالب بالا هستند فقط با هاست و کاربر خودشان. رکوردهای MX/SPF
را در پنل DNS (آروان) اضافه می‌کنید.
...
تست ارسال از خود سرور:

```bash
cd /srv/oramshop
sudo -u www-data .venv/bin/python manage.py shell -c "from django.core.mail import send_mail; send_mail('تست', 'سلام از اورام‌شاپ', None, ['ADDRESS@example.com'])"
```

اگر بدون خطا برگشت و ایمیل رسید (پوشه Spam را هم چک کنید)، تنظیم است. سپس در
پنل ← تنظیمات سایت، فیلد «ایمیل» را هم به آدرس نمایشی دلخواه تغییر دهید.
